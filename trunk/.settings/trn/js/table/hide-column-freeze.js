/*
* HideColumnFreeze V.0.2
* var hideColumnX = new HideColumnFreeze();
* hideColumnX.start();
*/

/**
 *  @desc: HideColumnFreeze constructor
 *  @type: public
 */
function HideColumnFreeze(){ //Contructor
 ...
 */
HideColumnFreeze.prototype.setExpireCookie = function(newDay){
 ...
 * @param newCookieName
 */
HideColumnFreeze.prototype.setCookieName = function(newCookieName){
	if(newCookieName!=""){
 ...
 */
HideColumnFreeze.prototype.addExcludeColumn = function(excludeClasses){
 ...
 */
HideColumnFreeze.prototype.setTitlebar = function(newText){
 ...
 */
HideColumnFreeze.prototype.setMsgAlertCookie = function(newMsg){
    this.msgAlertCookie = newMsg;
};

/**
* Start HideColumnFreeze 
* @type: public
**/
HideColumnFreeze.prototype.start = function(){
	var HideColumnFreeze = this;
	jQuery(function(){ //DOM ready
		
		//1. Create Panel 
		jQuery("table.HIDE_HEADER").each(function(tableIndex){ //loop สร้าง Panel ตามตารางที่มี
			var myPanel = jQuery('<div id="panel_hide_table_'+tableIndex+'" class="panel_hide_table_" title="'+HideColumnFreeze.titlebarPanel+'"></div>').appendTo('body');
			...
			jQuery("table.HIDE_HEADER:eq("+tableIndex+")").find('th').each(function(colNo, element){
				...
				for (var i=0; i<HideColumnFreeze.excludeColumns.length; i++) {
					if(that.hasClass(HideColumnFreeze.excludeColumns[i])){
				...
				myCheckbox.click(function(event){
					...
					HideColumnFreeze.doHideShow(tableIndex, colNo, this.checked);
					HideColumnFreeze.adjustWidth(tableIndex);
					HideColumnFreeze.saveToCookie(tableIndex);
				});
			});
			//set to dialog
		...
		jQuery('table.HIDE_HEADER').tooltip({ 
			...
			jQuery("body").on("click", "span#iconHideColumn_"+tableIndex, function(event){
				HideColumnFreeze.openPanel(tableIndex, event);
			});
		});
		
		
		//2. Hiding Column Onload page
		var cookie = HideColumnFreeze.getCookieValue(HideColumnFreeze.cookieName);
		if(cookie != ""){
...
HideColumnFreeze.prototype.adjustWidth = function(tableIndex){	
...
**/
HideColumnFreeze.prototype.doHideShow = function(tableIndex, colNo, doShow){
	var columnHead = jQuery('table.HIDE_HEADER:eq('+tableIndex+') tr > th:nth-child('+colNo+'):not(table)');
...
**/
HideColumnFreeze.prototype.getCookieValue = function (cname){
...
**/
HideColumnFreeze.prototype.saveToCookie = function(tableIndex){
	var HideColumn = this;
...
HideColumnFreeze.prototype.openPanel = function(tableIndex, event){
