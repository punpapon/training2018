/*--------------------------------------------------------------------
Scripts for creating and manipulating custom menus based on standard <ul> mark up
...
jQuery.fn.menufgDrilldown = function(options){
	var caller = this;
	var options = options;
	var m = new MenuDrilldown(caller, options);
	allUIMenus.push(m);
	m.create();
	m.kill();
	jQuery(this)
...
function MenuDrilldown(caller, options){
	var menu = this;
	...
	this.kill = function(){
		...
		container.parent().hide().unbind('click');
		menu.menuOpen = false;
...
MenuDrilldown.prototype.flyout = function(container, options) {
	var menu = this;
	...
	container.addClass('fg-menu-flyout').find('li:has(ul)').each(function(){
		...
		jQuery(this).find('a:eq(0)').addClass('fg-menu-indicator').html('<span>' + jQuery(this).find('a:eq(0)').text() + '</span><span class="ui-icon '+options.nextMenuLink+'"></span>').hover(
			function(){
				...
				if (!fitHorizontal(subList, jQuery(this).offset().left + 100)) { subList.css({ left: 'auto', right: linkWidth, 'z-index': 1000002 }); };
...
MenuDrilldown.prototype.drilldown = function(container, options) {
	var menu = this;
		...
		if (jQuery(this).next().is('ul')) {
			jQuery(this)
				...
				.click(function(){ // ----- show the next menu
					...
					var getLayerCount = function(element) {
						var layerCount = 1;
						var currentLayer = element;
						while (!jQuery(currentLayer).parents('ul:eq(0)').is('.fg-menu-content')) {
							layerCount++;
							currentLayer = jQuery(currentLayer).parents('ul:eq(0)')[0];
						}
						return layerCount;
					};
					...
					topList.animate({ left: -(getLayerCount(this) * container.width()) }, menu.menuOpen ? options.crossSpeed : 0);
		    		...
		    		var getNumToLoop = function (clickedATag) {
		    			var numToLoop = jQuery(".fg-menu-breadcrumb li").last().index() - jQuery(clickedATag).parent().index();
		    			return numToLoop;
		    		};
						...
						newCrumb
							...
							.find('a').click(function(){
								if (jQuery(this).parent().is('.fg-menu-current-crumb')){
									...
									var newLeftVal = - (jQuery(this).parent().index() * options.width);
									topList.animate({ left: newLeftVal  }, options.crossSpeed, function(){

									});
									var tmpLoop = getNumToLoop(currentA[0]);
									for (var i = 0 ; i < tmpLoop ; i ++) {
										setPrevMenu();
									}
									jQuery(this).parent().addClass('fg-menu-current-crumb').find('span').remove();
									jQuery(this).parent().nextAll().remove();
									/*
									var newLeftVal = - (jQuery('.fg-menu-current').parents('ul').size() - 1) * options.width;
									topList.animate({ left: newLeftVal }, options.crossSpeed, function(){
										setPrevMenu();
									});
									*/
...
MenuDrilldown.prototype.setPosition = function(widget, caller, options) {
	var el = widget;
	var referrer = caller;
	var dims = {
	...
	if (options.positionOpts.directionV == 'up') {
		...
		el.css({ bottom: 'auto', top: yVal, height: options.maxHeight + 50 });
		if (options.positionOpts.detectV && !fitVertical(el)) {
			el.css({ top: 'auto', bottom: yVal });
		}
	};
